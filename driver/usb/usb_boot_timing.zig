...
pub const MSC_POST_CONFIG_SETTLE_MS: u32 = 1_000;
...
test "boot retry windows remain bounded" {
    ...
    try testing.expect(MSC_POST_CONFIG_SETTLE_MS >= 500);
    try testing.expect(XHCI_EVENT_TIMEOUT_MS >= MSC_POST_CONFIG_SETTLE_MS);
}
